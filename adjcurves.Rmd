...
We consider a fictive study population of 10,000 patients who receive a certain medication. 60% of the patients are women. Women have a three times higher chance of receiving the medication compared to men. Patients who received the medication have a 0.5 lower hazard of dying compared to those without medication, but jointly, women have 10 times higher hazard of death compared to men.



In brief, IP weighting constructs weights which are equal to the probability of an individual's treatment given observed covariates and creates pseudopopulations in which observed covariates are independent of treatment (i.e. no confounding) (@hernan_book).
...
# Women have a 
medi <- ifelse(runif(n) < plogis(log(3)*female), 1, 0)
# Hazard of dying
haz <- 0.01*exp(log(0.5)*medi+log(10)*female)
# Time to death
...
# Data
data <- data.frame(fup, death, medi, female)
```
...
coxph(Surv(fup, death) ~ medi+female, data = data)
```
...
# IPW denominator
mod <- glm(medi ~ female, data=data, family=binomial())
summary(mod)
...
table(medi=data$medi, female=data$female)
svytable(~medi+female, design=svy_design)
