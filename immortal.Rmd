...
Immortal time bias arises when individuals are assigned to a treatment or exposure group in which they inherently survive for a specific time period (they are immortal). Immortal time bias is common when analyzing observational studies (@suissa, @levesque). It might occur when, for example, treatment or exposure group assignment happens after time zero (@hernan).  Other terminology for immortal time bias is *time dependent bias* or *survival bias*. In this vignette we introduce several methods to address immortal time bias (@zhou, @robins).

# Example

We consider a study population of 10,000 patients admitted to the emergency unit. Patients with a more severe admission status are treated more quickly and are grouped into two treatment groups (short and long time to treatment after emergency admission).
